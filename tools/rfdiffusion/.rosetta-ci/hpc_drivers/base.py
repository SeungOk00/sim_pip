...
# HPC 드라이버 공통 베이스:
# - 실행 유틸(execute, Sleep)
# - 잡 제출/대기/취소를 위한 추상 인터페이스(HPC_Driver)
# - 시그널(SIGINT/SIGTERM) 수신 시 남은 HPC 잡 정리
...
class NT:  # named tuple
    # 단순 attribute 컨테이너. (collections.namedtuple 대체 용도로 사용)
...
def execute(message, command_line, return_='status', until_successes=False, terminate_on_failure=True, silent=False, silence_output=False, tracer=print):
    # 공통 쉘 실행 함수:
    # - return_='tuple'  => (exit_code, output)
    # - return_='output' => output 문자열
    # - 그 외            => 실패 여부(bool-like)
    # until_successes=True면 성공할 때까지 재시도한다.
...
class HPC_Driver:
    def __init__(self, working_dir, config, tracer=lambda x:None, set_daemon_message=lambda x:None):
        # working_dir: 드라이버가 임시 스크립트/로그를 생성하는 기준 디렉터리
        # config: dict 또는 configparser 섹션 객체
        ...
        # 비정상 종료 시 남은 잡을 취소할 수 있도록 signal handler 설치
    ...
    def submit_serial_hpc_job(self, name, executable, arguments, working_dir, jobs_to_queue, log_dir, memory=512, time=12, block=True, shell_wrapper=False):
        # 구현체(MultiCore/Slurm)가 실제 제출 로직을 제공해야 한다.
        must_be_implemented_in_inherited_classes
    ...
    def wait_until_complete(self, jobs=None, callback=None, silent=False):
        ...
        # complete(job)가 True가 될 때까지 주기적으로 확인한다.
    ...
    def install_signal_handler(self):
        def signal_handler(signal_, frame):
            # Ctrl+C 등 인터럽트 발생 시 현재 제출한 잡을 정리
