...
#
# SLURM 기반 HPC 드라이버:
# - sbatch 스크립트 생성 후 array/MPI job 제출
# - squeue로 완료 여부 확인, scancel로 취소
...
'''
# 동일 명령을 N개 반복 실행할 때 사용하는 array job 템플릿
...
class Slurm_HPC_Driver(HPC_Driver):
    def head_node_execute(self, message, command_line, *args, **kwargs):
        # 헤드 노드가 지정되어 있으면 ssh를 통해 해당 노드에서 명령 수행
    ...
    def submit_serial_hpc_job(self, name, executable, arguments, working_dir, jobs_to_queue, log_dir, memory=512, time=12, block=True, shell_wrapper=False):
        ...
        # 입력 time(시간)을 분 단위 정수로 변환

        if shell_wrapper:
            # 실행 커맨드를 wrapper 스크립트로 감싸서 제출
        ...
        # sbatch 출력("Submitted batch job <id>")에서 job id 추출
